...
export default function AdminPage() {
  ...
  const inProduction = adminClients.filter((c) => c.websiteStatus === "In production").length;
  const waitingForClient = adminClients.filter((c) => c.websiteStatus === "Waiting for client").length;
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#0B0F19] px-6 pb-24 pt-32 text-white">
      ...
      >
        {[
          { label: "Total clients", value: adminClients.length },
            ...
            >
              <SpotlightCard className="grid gap-6 rounded-2xl border border-white/10 bg-white/[0.03] p-5 transition-colors hover:border-cyan-400/20 lg:grid-cols-[1fr_1fr_auto] lg:items-center">
                <div>
                  ...
                  <p className="mt-2 text-sm text-zinc-400">Client: {client.clientName}</p>
                </div>

                <div className="grid gap-3 sm:grid-cols-2">
                  <div>
                    <p className="mb-1 text-xs text-zinc-500">Website status</p>
                    <p className="text-sm font-medium text-white">{client.websiteStatus}</p>
                  </div>
                  <div>
                    <p className="mb-1 text-xs text-zinc-500">Current stage</p>
                    <p className="text-sm font-medium text-white">{client.currentStage}</p>
                  </div>
                  <div>
                    <p className="mb-1 text-xs text-zinc-500">Next step</p>
                    <p className="text-sm font-medium text-white">{client.nextStep}</p>
                  </div>
                  <div>
                    <p className="mb-1 text-xs text-zinc-500">Last update</p>
                    <p className="text-sm font-medium text-white">{client.lastUpdate}</p>
                  </div>
                </div>
